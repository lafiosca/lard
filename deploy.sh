...
aws cloudformation deploy --template-file sam-output.yml --stack-name "$StackName" --capabilities CAPABILITY_IAM --parameter-overrides CodePipelineS3Bucket="$CodePipelineS3Bucket" CreateCodePipelineCloudFormationAdminRole="$CreateCodePipelineCloudFormationAdminRole"
